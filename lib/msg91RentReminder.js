...
function resolveConfig() {
  return {
    ...
    dueDay: Number(process.env.MSG91_RENT_REMINDER_DUE_DAY || ""),
  };
}
...
function getBillingMonthForDueDate(year, monthIndex) {
  const billingDate = new Date(year, monthIndex, 1);
  billingDate.setMonth(billingDate.getMonth() - 1);
  return {
    year: billingDate.getFullYear(),
    monthIndex: billingDate.getMonth(),
    monthKey: formatMonthKey(billingDate.getFullYear(), billingDate.getMonth()),
  };
}
...
function getUpcomingDueContext(tenant, now = new Date()) {
  ...
  const { dueDay: configuredDueDay } = resolveConfig();
  const joinDay =
    joinDate && !Number.isNaN(joinDate.getTime()) ? joinDate.getDate() : null;
  const anchorDay = Number.isFinite(configuredDueDay) && configuredDueDay > 0
    ? configuredDueDay
    : joinDay;
  ...
  const billingMonth = getBillingMonthForDueDate(year, monthIndex);

  return {
    dueDate,
    dueDay,
    dueMonthKey: formatMonthKey(year, monthIndex),
    monthKey: billingMonth.monthKey,
    billingYear: billingMonth.year,
    billingMonthIndex: billingMonth.monthIndex,
